...
async def perform_comprehensive_search(
    ...
    """Perform vector + hybrid search and return combined results."""
    results = {
        "query": query,
        "vector_results": [],
        "total_results": 0
    ...
    if use_vector:
        vector = await vector_search_tool(VectorSearchInput(query=query, limit=limit))
        results["vector_results"] = vector

    results["total_results"] = len(results["vector_results"])
    return results