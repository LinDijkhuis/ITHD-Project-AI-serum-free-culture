...
@dataclass
class DocumentChunk:
    """Represents a document chunk with optional section label."""
    content: str
    index: int
    start_char: int
    end_char: int
    ...
    # Section name from IMRaD detection (e.g. "methods", "results"); None for non-PDF docs
...
class SemanticChunker:
    ...
    async def chunk_document_sections(
        ...
        """
        Chunk a paper section by section so chunks never cross section boundaries.

        Each chunk is tagged with the section name (e.g. "methods", "results")
        in both its metadata dict and its .section attribute. This lets the
        retrieval agent filter or boost results by section.

        Args:
            sections:  List of PaperSection objects from pdf_parser.parse_pdf()
            title:     Paper title (added to every chunk's metadata)
            source:    Relative file path (added to every chunk's metadata)
            metadata:  Additional metadata to merge into every chunk

        Returns:
            Flat list of DocumentChunk objects, re-indexed globally across all sections
        """
        all_chunks: List[DocumentChunk] = []

        for section in sections:
            # Chunk this section's content independently
            section_chunks = await self.chunk_document(
                ...
                metadata={
                    **(metadata or {}),
                    "section": section.name,           # Normalized name, e.g. "methods"
                    "section_heading": section.heading, # Original text, e.g. "3. Materials and Methods"
                    "page_start": section.page_start,
                },
            )
            # Tag the section field on each chunk and assign a global index
            for chunk in section_chunks:
                chunk.section = section.name
                chunk.index = len(all_chunks)
                all_chunks.append(chunk)

        # Now we know the final total — update metadata on every chunk
...
class SimpleChunker:
    ...
    async def chunk_document_sections(
        ...
        """
        Chunk a paper section by section (simple/fast version).

        Mirrors SemanticChunker.chunk_document_sections() so both chunkers
        ...
        Args:
            ...
            title:     Paper title
            source:    Relative file path
            metadata:  Additional metadata merged into every chunk

        Returns:
            Flat list of DocumentChunk objects with section labels
        """
        all_chunks: List[DocumentChunk] = []
