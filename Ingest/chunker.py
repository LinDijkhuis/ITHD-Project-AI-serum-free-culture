...
class ChunkingConfig:
    ...
    # Changed for more precise chunks.
    chunk_size: int = 800
...
class SemanticChunker:
    ...
    def _split_on_structure(self, content: str) -> List[str]:
        ...
        Returns:
            List of sections
        """
         # Split on scientific paper section headers as primary split boundaries 
        section_headers = [
            r'\n(?:Abstract|Introduction|Methods?|Materials and Methods?|'
            r'Results?|Discussion|Conclusion|References?|'
            r'Cell Culture|Media Composition|Supplementary)\b',
    ]
    ...
    def _detect_section(self, chunk_text: str) -> str:
        """Detect which paper section a chunk belongs to."""
        section_keywords = {
            "abstract": ["abstract"],
            "methods": ["methods", "materials and methods", "cell culture",
                        "media", "medium", "culture conditions"],
            "results": ["results", "figure", "table"],
...
async def main():
    ...
    sample_text = """ 
        # Cell culturing methods are increasingly used to reduce and 
        # replace the use of live animals in biomedical research and 
        # chemical toxicity testing.
    """
    
    chunks = await chunker.chunk_document(
        content=sample_text,
        title="A new animal product free defined medium for 2D and 3D culturing of normal and cancer cells",
        source="example.pdf"
    )
